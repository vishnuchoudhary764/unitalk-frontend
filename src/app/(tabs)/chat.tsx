...
// /* ─────────────────────────────────────────
//    EMOJI DATA  (6 categories × ~30 emojis)
// ───────────────────────────────────────── */
// const EMOJI_CATS = [
//   {
//     label: "😊",
...
//         setIsTyping(false);
//       });
//       socket.on("friend-request-sent", () => setStatus("Friend request sent ❤️"));
//       socket.on("friend-request-received", () => setStatus("📩 Friend request received"));
//       socket.on("friend-request-accepted", ({ friendId }: { friendId: string }) => {
//         disconnectSocket();
//         router.replace({ pathname: "/private-chat", params: { friendId } });
//       });
...
//   const sendFriendRequest = () => socketRef.current?.connected && socketRef.current.emit("send-friend-request");
//   const endChat = () => {
...
//                 <View style={[
//                   s.bubble,
//                   item.me ? s.bubbleMe : s.bubbleThem,
//                 ]}>
//                   {item.me ? (
...
//     shadowColor: "#7C5CFC", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.07, shadowRadius: 4, elevation: 1,
//   },
...
export default function Chat() {
  ...
  // toast for transient messages (friend request sent, already sent, etc.)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      /* ── FIX: already-friends → redirect to private chat ── */
